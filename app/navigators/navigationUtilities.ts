...
type PersistNavigation = "always" | "dev" | "prod";
/**
 ...
 */
function navigationRestoredDefaultState(persistNavigation: PersistNavigation) {
...
export function useNavigationPersistence(persistenceKey: string) {
  ...
  const onNavigationStateChange: NavigationProps["onStateChange"] = state => {
    ...
    clientStorage.save(persistenceKey, state);
  };

  const restoreState = async () => {
    try {
      const state = clientStorage.load(persistenceKey) as
